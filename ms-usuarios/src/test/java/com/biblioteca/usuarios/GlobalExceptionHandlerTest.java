...
import com.biblioteca.usuarios.application.dto.ApiResponseDTO;
...
class GlobalExceptionHandlerTest {
    ...
    void handleRuntime_retorna400() {
        ResponseEntity<ApiResponseDTO<Void>> response = handler.handleRuntime(new RuntimeException("error negocio"));
        assertThat(response.getStatusCode().value()).isEqualTo(400);
        assertThat(response.getBody().getExito()).isFalse();
        assertThat(response.getBody().getMensaje()).isEqualTo("error negocio");
    }
    ...
    void handleValidation_retorna400ConMensajes() {
        ...
        FieldError fieldError = new FieldError("obj", "nombre", "no debe estar en blanco");
    ...
    void handleGeneral_retorna500() throws Exception {
        ResponseEntity<ApiResponseDTO<Void>> response = handler.handleGeneral(new Exception("fallo inesperado"));
